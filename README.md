...
```
In main branch: Open another terminal in root folder to run this app in local using ```json server``` database by installing ```json-server``` In branch ```firestore-db``` no need json-server to watch, only npm installation.
```
